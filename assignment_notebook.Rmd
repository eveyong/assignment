...
library(knitr)
library(kableExtra)
knitr::kable(head(data_monthly_raw)) %>%
...
library(knitr)
library(kableExtra)
knitr::kable(miss_var_summary(data_monthly_raw)) %>%
  kable_styling(latex_options = c("striped", "scale_down"),
                font_size = 10)
...
library(knitr)
library(kableExtra)
knitr::kable(miss_var_summary(data_yearly_raw)) %>%
  kable_styling(latex_options = c("striped", "scale_down"),
                font_size = 10)
```
...
par(mfrow=c(1,2))
plot(test$average_price,pr.nn.rescaled,col='red',main='Best Model: Predicted NN vs real',pch=18,cex=0.7, xlab = "Average house price (£)", ylab = "Predicted")
abline(0,1,lwd=2)
legend('bottomright',legend='NN',pch=18,col='red', bty='n')
