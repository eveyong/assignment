...
We first split the data into train and test data using a 90%/10% split.
...
```
Mean CV error for NN is 6bn, which is lower than GLM CV error, 17bn, although there seems to be a certain degree of variation in the MSEs of the cross validation. MSE for the NN machine learning applied in our model was 3bn compared to GLM's MSE of 10bn. This shows that the neural network was a better predictor of average house prices compared to GLM.
...
```{r}
cat("MSE of regression tree =", rmse_reg(tree, test_tree, "average_price")^2, "\n")
```

If we grow an overly complex tree as in the tree diagram above, there tends to be overfitting to the training data, which results in poor generalisation performance.

To find the balance of depth and complexity of the tree, there are two approaches - pruning and early stopping.

We explore the pruning option below.
...
plotcp(tree,minline=TRUE,
       col = "red")
```
From the above diagram, we can see optimal complexity parameter is roughly 0.007. We prune the tree to return a smaller optimal tree. We calculate the optimal complexity parameter by using the 1SD rule as well to double check.
...
plot(yhat, actual, xlab = "Predicted", ylab = "Actual", main = "Predicted vs Actual")
abline(0,1)
...
cat("MSE of regression tree after pruning =", rmse_reg(tree_pruned, test_tree, "average_price")^2, "\n")
```

The MSE is slightly higher than the previous tree, but still higher than that of the neural network model, which was 3bn. 
...
We calculate the MSE using CARET
```{r}
rmse_reg(train.rpart, test_tree, "average_price")^2
```
The MSE has greatly reduced to less than 1bn, even more effective than the previous NN model.

However, the diagram below shows that there are many leaves compared to the unpruned tree.
...
plot(yhat2, actual2, xlab = "Predicted", ylab = "Actual", main = "Predicted vs Actual")
abline(0,1)
```
...
#build the model with two densely connected hidden layers with one single continuous layer as output
#create function for the model
build_model <- function() {
  model <- keras_model_sequential() %>%
    layer_dense(units = 64, activation = "relu",
                input_shape = dim(train_scaled)[2]) %>%
    layer_dense(units = 64, activation = "relu") %>%
    layer_dense(units = 1)
...
train_tree$fit <- train_tree$average_price*predict(tree_pruned)
test_tree$fit <- test_tree$average_price*predict(tree_pruned, newdata=test_tree)

#calculate in sample and our of sample error based on deviance loss function
in_sample_err<-2*(sum((train_tree$average_price/train_tree$fit))-sum(train_tree$average_price)+sum(train_tree$fit))/nrow(train_tree)
out_sample_err<-2*(sum((test_tree$average_price/test_tree$fit))-sum(test_tree$average_price)+sum(test_tree$fit))/nrow(test_tree)

cat("in-sample error using deviance loss fuction =", in_sample_err, "\n")
cat("out-of-sample error using deviance loss function =", out_sample_err, "\n")

```

```{r}
train_tree$fit<-train_tree$fit*predict(boosting.step)^shrinkage
    test_tree$fit<-test_tree$fit*predict(boosting.step, newdata=test_tree)^shrinkage

    in_sample_error[k]<-2*(sum((train_tree$average_price/train_tree$fit))-sum(train_tree$average_price)+sum(train_tree$fit))/nrow(train_tree)
    out_sample_error[k]<-2*(sum((test_tree$average_price/test_tree$fit))-sum(test_tree$average_price)+sum(test_tree$fit))/nrow(test_tree)
```

```{r}
#perform boosting machine with SBS tree with K=2 leaves only
idepth<-30
...
for(k in 1:K){
    boosting.step <- rpart(average_price ~ .,
                         data=train_tree,
                         method="anova", #not Poisson nor discrete so choose anova
                         control = rpart.control(maxsurrogate=0, xval=1,cp=0.007,maxdepth=idepth))
    
    rmse_reg_err[k] <- rmse_reg(boosting.step, test_tree, "average_price")^2
    
}

par(mfrow=c(1,2))
plot(seq(1:K),rmse_reg_err, xlab="iteration", ylab="MSE")
