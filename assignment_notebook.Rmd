...
author: "Last name: Yong"
date: "First name: Wen Qian"
header-includes: 
- \usepackage{fancyhdr}
- \pagestyle{fancy}
...
- \fancyfoot[R]{\thepage}
subtitle: "Assessment Task: Project |  github: https://github.com/eveyong/assignment"
...
The next section looks at exploration and data analysis using this joined data.
...
```
The MSE has actually increased which means it is less accurate compared to pruning it until the optimal cp-value. Just like the original tree, the diagram below shows that there are many leaves compared to the unpruned tree. 
...
The plot below shows the predicted versus actual average house prices using the CARET tree model. 
...
Neural Network model did the best job in predicting average house prices in London. Multiple Linear Regression and Decision Tree was not as good at predicting the average house prices. Decision tree without pruning and using CARET resulted in too many influencers that are no good for predictions.
...
Finally, we tried letting the decision tree model streamline the model building and evaluation process (via the CARET package). This made the model accuracy worse so in the end pruning the decision tree was the best we could do in relation to decision trees. 
