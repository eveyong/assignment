...
We first split the data into train and test data using a 70%/30% split.
...
trainsampleindex <- sample(1:nrow(data_NN_complete), 0.7*nrow(data_NN_complete))
...
```
Therefore we normalise the features in the training data before applying any model fit to it.
...
nn <- neuralnet(f,data=train_scaled, hidden=c(4,2),act.fct = 'tanh',linear.output=T, stepmax=1e7)
...
pr.nn.rescaled <- pr.nn$net.result * (max(data_NN_complete$average_price)-min(data_NN_complete$average_price)) + min(data_NN_complete$average_price)
test.rescaled <- test_scaled$average_price * (max(data_NN_complete$average_price)-min(data_NN_complete$average_price)) + min(data_NN_complete$average_price)


par(mfrow=c(1,2))
plot(test$average_price,pr.nn.rescaled,col='red',main='predicted NN vs real',pch=18,cex=0.7)
abline(0,1,lwd=2)
